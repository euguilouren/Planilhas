...
def config_completo():
    return {
        "pastas": {"entrada": "pasta_entrada", "saida": "pasta_saida", "log": "pasta_saida/log.txt"},
        "colunas": {
            "valor": "Valor",
            "categoria": "Categoria",
            "data": "Data",
...
class TestLeitor:
    ...
    def test_resumo_diagnostico_retorna_string(self):
        diag = {
            ...
            "total_registros": 10,
            "abas": [
                {"nome": "Dados", "linhas": 10, "colunas": ["A", "B"], "nulos": {"A": 0, "B": 1}, "duplicatas": 0}
            ],
            "problemas_formato": [],
        }
...
class TestInconsistenciasTemporais:
    ...
    def test_data_invertida_detectada(self):
        ...
        resultado = Auditor.detectar_inconsistencias_temporais(df, "Emissao", col_data2="Vencimento")
        tipos = [r["tipo"] for r in resultado]
...
class TestNormalizadorTipoInferido:
    ...
    def test_tipo_inferido_da_categoria_receita(self):
        ...
        result = Normalizador.para_padrao(
            df,
            {
                "NF": "NF",
                "Data": "Data",
