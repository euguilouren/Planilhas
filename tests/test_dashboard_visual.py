...
class TestGeradorDashboard:
    ...
    def test_banner_sem_criticos(self, df_dados, df_mensal):
        html = GeradorDashboard.gerar("arquivo.xlsx", df_dados, df_fluxo_mensal=df_mensal, total_criticos=0)
        assert "dados prontos para apresentação" in html

    def test_banner_com_criticos(self, df_dados, df_mensal):
        html = GeradorDashboard.gerar("arquivo.xlsx", df_dados, df_fluxo_mensal=df_mensal, total_criticos=3)
    ...
    def test_arquivo_nome_aparece(self, df_dados, df_mensal):
        html = GeradorDashboard.gerar("relatorio_jan.xlsx", df_dados, df_fluxo_mensal=df_mensal)
    ...
    def test_dre_opcional_aparece(self, df_dados, df_mensal):
        ...
        html = GeradorDashboard.gerar("arquivo.xlsx", df_dados, df_fluxo_mensal=df_mensal, df_dre=df_dre)
    ...
    def test_pareto_opcional_aparece(self, df_dados, df_mensal):
        ...
        html = GeradorDashboard.gerar("arquivo.xlsx", df_dados, df_fluxo_mensal=df_mensal, df_pareto=df_pareto)
        assert "Alfa" in html
        assert "Beta" in html
